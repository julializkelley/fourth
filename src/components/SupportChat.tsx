...
export function SupportChat() {
  ...
  // landing on the page doesn't count as "in distress right now".
  useEffect(() => {
  ...
  return (
    <div className="support-chat" id="support-chat">
      <div className="support-chat-head">
        <div>
          <h3>
            {mode === "paired" ? "Anonymous chat" : mode === "ended" ? "Chat ended" : "Talk to someone, right now"}
          </h3>
          <p>
            {mode === "ai" && "Free, anonymous, no account needed — support & education, not a substitute for a professional"}
      ...
      <div className="support-chat-messages" ref={scrollRef}>
        {mode === "ai" &&
          (aiMessages.length === 0 ? (
            <div className="chat-bubble chat-bubble-system support-chat-greeting">
              You&rsquo;re free to let it out here. Vent about what&rsquo;s going on, get general
              advice, or even connect with another mom who&rsquo;s also here right now.
            </div>
          ) : (
            aiMessages.map((m, i) => (
              <div key={i}>
        ...
        {mode === "ended" && (
          <div className="chat-bubble chat-bubble-system">
            {endedReason === "risk_language" ? "This chat was ended for everyone's safety." : "Chat ended."}
          </div>
        )}
